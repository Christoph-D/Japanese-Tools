...
class KakasiController(object):
    ...
    def toHiragana(self, expr):
        self.ensureOpen()
        self.kakasi.stdin.write(expr.encode("utf8", "ignore") + b'\n')
...
def fixExpr(expr):
    ...
    for node in expr_split:
        ...
        if kanji.strip() in "０１２３４５６７８９": # "一二三四五六七八九十０１２３４５６７８９":
            out.append(kanji)
            continue
...
def get_readings(expr):
    try:
        mecab = subprocess.Popen(
            ...
            stdout=subprocess.PIPE)
        return mecab.communicate(expr.encode('utf-8'))[0].decode('utf-8')
    except OSError:
